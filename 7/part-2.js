...
function count(hand) {
	...
	let countedHand = hand.reduce((count, card) => ({...count, [card]: count[card] ? ++count[card] : 1}), {})
...
function uniques(hand, occurances) {
	...
	return Object.entries(counted).filter(([card, counts]) => card !== "J" && counts === occurances).length
}
